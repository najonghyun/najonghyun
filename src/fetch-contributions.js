...
export async function fetchContributions(username, token = null, options = {}) {
    ...
    // 배열로 변환하고 PR 수 기준 정렬
    const contributions = Array.from(repoMap.values()).sort((a, b) => b.prs.length - a.prs.length);
...
export async function fetchFeaturedPRs(prList, token = null) {
    ...
    // 병렬로 API 호출
    const results = await Promise.allSettled(parsed.map(({ owner, repo, prNumber }) => fetchSinglePR({ owner, repo, prNumber, headers })));
